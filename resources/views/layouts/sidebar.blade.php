<div class="h-full bg-white border-r border-gray-200 w-64 fixed top-0 left-0 z-20 flex flex-col transition-transform duration-300">
    ...
    <div class="flex-1 overflow-y-auto py-6 px-4 space-y-1 relative">
        ...
        @if(auth()->user()->role === 'pelatih')
            ...
            <a href="{{ route('pelatih.profile.index') }}" class="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition-colors group {{ request()->routeIs('pelatih.profile.*') ? 'bg-orange-50 text-orange-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                <svg class="w-5 h-5 mr-3 {{ request()->routeIs('pelatih.profile.*') ? 'text-orange-600' : 'text-gray-400 group-hover:text-gray-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                Profil Pelatih
            </a>
        @endif
    </div>
